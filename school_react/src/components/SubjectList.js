...
function StudentList() {
    ...
    const handleEdit = async (id) => {
      ...
      const handleDelete = async (id) => {
        try {
          ...
          const response = await fetch(`/SCM/subject/delete/${id}/`, {
            method: 'DELETE'
          });
    ...
    return (
        <div className='table-container'>
          <div className='row'>
          <h2>Subject Table</h2>
            <div className='col-md-3' style={{marginRight:'-100px',marginTop:'10px'}}><Link  className='buttonLink' to="/subject/add">Add<i className="fa fa-plus" aria-hidden="true"></i></Link>
            </div>
          ...
          <TableBody>
            ...
            {currentItems.map((item, index) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.subjectDescription}</TableCell>
