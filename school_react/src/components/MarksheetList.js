...
function MarksheetList() {
    ...
    const handleEdit = async (id) => {
      ...
      const handleDelete = async (id) => {
        try {
          ...
          const response = await fetch(`/SCM/marksheet/delete/${id}/`, {
            method: 'DELETE'
          });
    ...
    const calculatePercentage = (physics, maths, chemistry) => {
        const totalMarks = physics + maths + chemistry;
        return ((totalMarks / 300) * 100).toFixed(2); // Assuming total marks are out of 300
    };
    return (
        <div className='table-container'>
          <div className='row'>
          <h2>Marksheet Table</h2>
            <div className='col-md-3' style={{marginRight:'-100px',marginTop:'10px'}}><Link  className='buttonLink' to="/marksheet/add">Add<i className="fa fa-plus" aria-hidden="true"></i></Link>
            </div>
          ...
          <TableBody>
            ...
            {currentItems.map((item, index) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.physics}</TableCell>
                <TableCell>{item.chemistry}</TableCell>
                <TableCell>{item.maths}</TableCell>
                <TableCell>{calculatePercentage(item.physics, item.maths, item.chemistry)}%</TableCell>
